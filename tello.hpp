...
#define TELLO_MOVE_COMMAND_TIMEOUT 1
...
public:
	...
	bool move(int left_right, int forward_back, int up_down, int yaw) {
...
private:
	...
	bool execute_command(const std::string& str, bool silent = false) {
		if (str.find("rc") != std::string::npos) {
			int dumen = TELLO_MOVE_COMMAND_TIMEOUT;
			return execute_command_raw(str, dumen, silent);
    	}
